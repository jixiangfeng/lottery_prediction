...
class DigitCandidateConfig:
    ...
    frequency_windows: tuple[int, ...] = (20, 50, 100, 300)
    ...
    def __post_init__(self) -> None:
        ...
        if not self.frequency_windows or any(int(window) <= 0 for window in self.frequency_windows):
...
class DigitBettingCandidateResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        legacy["directCandidates"] = [candidate.to_dict() for candidate in self.direct_candidates]
        legacy["groupCandidates"] = [candidate.to_dict() for candidate in self.group_candidates]
        return legacy
...
def _weighted_log_probability(
    ...
    weighted = 0.0
    total_weight = 0.0
    for window, weight in zip(config.frequency_windows, config.frequency_window_weights):
        ...
        if probabilities is None or positive_weight == 0:
            continue
        weighted += positive_weight * math.log(max(probabilities.get(value, 0.0), 1e-300))
...
def _build_score_context(stats: DigitStatisticsResult, config: DigitCandidateConfig) -> dict[str, Any]:
    ...
    omission = {
        position: {
            digit: math.log1p(min(miss, omission_cap)) / math.log1p(omission_cap)
            for digit, miss in stats.current_omission.get(position, {}).items()
        }
        for position in positions
    }
    return {
...
def _score_prefix_with_context(
    ...
    marginal_logs = [context["marginal"][positions[index]][int(numbers[index])] for index in range(3)]
    pair_logs = [
...
def _score_numbers_with_context(
    ...
    suffix_marginals = [context["marginal"][positions[index]][int(numbers[index])] for index in suffix_indexes]
    suffix_pairs = [
        context["pair"][f"{left}-{right}"][(int(numbers[left]), int(numbers[right]))]
...
def _omission_score(
    ...
    for index, digit in zip(indexes, numbers):
        omission = stats.current_omission.get(positions[index], {})
        scores.append(math.log1p(min(omission.get(int(digit), 0), omission_cap)) / math.log1p(omission_cap))
...
def score_digit_prefix(
    ...
    return _score_prefix_with_context(numbers, config, _build_score_context(stats, config))
...
def score_digit_numbers(
    ...
    return _score_numbers_with_context(numbers, config, _build_score_context(stats, config))


def _digit_score(stats: DigitStatisticsResult, position: str, digit: int, config: DigitCandidateConfig) -> float:
...
def _effective_config(rule: LotteryRule, config: DigitCandidateConfig) -> DigitCandidateConfig:
    ...
    if rule.draw_count == 5:
        return DigitCandidateConfig(
            ...
            span_max=9 if config.span_max is None else config.span_max,
            allowed_shapes=("全不同", "二一一一", "二二一", "三一一", "三二") if config.allowed_shapes is None else config.allowed_shapes,
            top_digits_per_position=config.top_digits_per_position,
            frequency_weight=config.frequency_weight,
...
@lru_cache(maxsize=8)
def _digit_universe(draw_count: int) -> tuple[tuple[tuple[int, ...], str, int, int, str], ...]:
    """缓存 0-9 数字彩号码空间的静态形态信息。"""

    rows = []
    for values in itertools.product(range(10), repeat=draw_count):
        text = "".join(str(number) for number in values)
        rows.append((values, text, sum(values), max(values) - min(values), classify_digit_shape(values)))
    return tuple(rows)


def _passes_cached_filters(sum_value: int, span: int, shape: str, config: DigitCandidateConfig) -> bool:
    if config.sum_min is not None and sum_value < config.sum_min:
        return False
    if config.sum_max is not None and sum_value > config.sum_max:
        return False
...
def _shape_budget(rule: LotteryRule, config: DigitCandidateConfig, count: int) -> dict[str, int]:
    """返回统计与随机策略共享的严格形态预算。"""
...
def _diversity_select(
    ...
    *,
    ...
    score_only_floor = ranked_candidates[min(count, len(ranked_candidates)) - 1].score
    configured_floor = best_score - config.score_floor
    quality_pool = [
        candidate for candidate in ranked_candidates if candidate.score >= configured_floor - 1e-12
    ]
    if len(quality_pool) < count:
        quality_pool = list(ranked_candidates[:count])
    else:
        quality_pool = [
            candidate for candidate in quality_pool if candidate.score >= score_only_floor - 1e-12
        ]
        ...
        for index, candidate in enumerate(remaining):
            ...
            if selected:
                for position, digit in enumerate(candidate.numbers):
                    position_repeats = sum(item.numbers[position] == digit for item in selected)
                    repeated_positions += position_repeats
                    if position_repeats == 0:
                        unseen_positions += 1
                nearest_similarity = max(
                    sum(left == right for left, right in zip(candidate.numbers, item.numbers)) / len(candidate.numbers)
                    for item in selected
                )
            raw_diversity = 2.0 * unseen_positions - repeated_positions - nearest_similarity
            diversity = max(-1.0, min(1.0, raw_diversity))
            same_score = round(candidate.score, 3)
            adjusted = same_score + config.diversity_weight * diversity
            key = (adjusted, candidate.score, "".join(chr(255 - ord(char)) for char in candidate.text))
...
def _enumerate_scored_candidates(
    ...
    rows: list[tuple[list[int], str, int, int, str, float]] = []
    ...
    for values, text, sum_value, span, shape in _digit_universe(rule.draw_count):
        ...
        if config.exclude_latest and text == latest_text_for_exclusion:
            continue
        score = _score_numbers_with_context(numbers, config, context)
        rows.append((numbers, text, sum_value, span, shape, score))
    if not rows:
        return []
    ...
    candidates = [
        ...
        for numbers, text, sum_value, span, shape, score in rows
    ]
    return sorted(candidates, key=lambda candidate: (-candidate.score, candidate.text))


_SCORED_POOL_CACHE: OrderedDict[tuple[int, str, DigitCandidateConfig], tuple[DigitStatisticsResult, list[DigitCandidate]]] = OrderedDict()
...
def _scored_candidate_pool(
    ...
    if cached is not None and cached[0] is stats:
        ...
        return cached[1]
    latest_text = _candidate_text(stats.latest_numbers) if stats.latest_numbers else None
...
def _order_three_digit_candidates_by_shape_probability(candidates: Sequence[DigitCandidate], config: DigitCandidateConfig) -> list[DigitCandidate]:
    """按形态概率重排三位数字彩候选。
...
def _order_pl5_candidates_by_shape_probability(
    ...
    defensive_shapes = {"三一一", "三二"}
    defensive = [candidate for candidate in candidates if candidate.shape in defensive_shapes]
    mainstream = [candidate for candidate in candidates if candidate.shape not in defensive_shapes]
    if not mainstream:
...
def generate_digit_candidates(
    ...
    *,
    ...
    _ = seed
    ranked = _scored_candidate_pool(stats, rule, config)
    budget = _shape_budget(rule, config, config.count)
    mainstream = [candidate for candidate in ranked if not _is_defensive_shape(rule, candidate.shape)]
    defensive = [candidate for candidate in ranked if _is_defensive_shape(rule, candidate.shape)]
    ...
    if rule.draw_count == 3:
        candidates = _order_three_digit_candidates_by_shape_probability(candidates, config)
    ...
    if len(candidates) != config.count:
        raise ValueError(f"候选不足：请求 {config.count} 注，过滤后只能生成 {len(candidates)} 注")
...
def generate_digit_betting_candidates(
    ...
    *,
    ...
    direct = generate_digit_candidates(stats, rule, config=config)
    if rule.draw_count != 3:
    ...
    requested_group_count = direct.config.count if group_count is None else int(group_count)
    ...
    ordered = _scored_candidate_pool(stats, rule, direct.config)
    ordered = [candidate for candidate in ordered if candidate.shape in {"组六", "组三"}]
    if not ordered:
        ...
        groups.sort(key=lambda candidate: (-candidate.probability_mass, candidate.group_key))
        ...
        groups = sorted(
            ...
            key=lambda candidate: (-candidate.probability_mass, candidate.group_key),
        )[:requested_group_count]
    if len(groups) != requested_group_count:
        raise ValueError(f"组选候选不足：请求 {requested_group_count} 注，过滤后只能生成 {len(groups)} 注")
    return DigitBettingCandidateResult(
        rule.code,
        rule.display_name,
        direct.candidates,
...
def rank_digit_numbers(
    ...
    rank, score, _ = rank_digit_numbers_with_eligible_count(stats, rule, numbers, config)
    return rank, score
...
def rank_digit_numbers_with_eligible_count(
    ...
    ranked = _scored_candidate_pool(stats, rule, effective)
    better = 0
    for candidate in ranked:
        score = candidate.score
        text = candidate.text
        if score > target_score + 1e-12 or (abs(score - target_score) <= 1e-12 and text < target_text):
            better += 1
    eligible = len(ranked)
    return (better + 1 if target_in_space else eligible + 1, target_score, eligible)
...
def generate_uniform_digit_candidates(
    ...
    *,
    ...
    if len(universe) < config.count:
        raise ValueError(f"候选不足：请求 {config.count} 注，过滤后只能生成 {len(universe)} 注")
    if rule.draw_count in {3, 5}:
        budget = _shape_budget(rule, config, config.count)
        defensive = [candidate for candidate in universe if _is_defensive_shape(rule, candidate.shape)]
        mainstream = [candidate for candidate in universe if not _is_defensive_shape(rule, candidate.shape)]
        if len(mainstream) < budget["mainstream"] or len(defensive) < budget["defensive"]:
            raise ValueError(f"候选不足：请求 {config.count} 注，过滤后无法满足形态预算")
...
def generate_uniform_digit_betting_candidates(
    ...
    *,
    ...
    if rule.draw_count != 3:
        return DigitBettingCandidateResult(rule.code, rule.display_name, direct.candidates, [], direct.config)
    ...
    for _, text, sum_value, span, shape in _digit_universe(rule.draw_count):
        if direct.config.exclude_latest and text == latest_text:
            continue
        if shape not in {"组六", "组三"} or not _passes_cached_filters(
            sum_value, span, shape, direct.config
        ):
            continue
        key = "".join(sorted(text))
        permutation_counts[key] += 1
        group_shapes[key] = shape
    ...
    groups = [
        DigitGroupCandidate(
            ...
            probability_mass=(permutation_counts[key] / total_permutations if total_permutations else 0.0),
            score=(math.log(permutation_counts[key] / total_permutations) if total_permutations else -math.inf),
            permutations=permutation_counts[key],
        )
    ...
    if len(groups) != requested:
        raise ValueError(f"组选候选不足：请求 {requested} 注，过滤后只能生成 {len(groups)} 注")
    return DigitBettingCandidateResult(rule.code, rule.display_name, direct.candidates, groups, direct.config)
