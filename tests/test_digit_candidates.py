...
def test_default_fc3d_candidates_exclude_latest_and_baozi():
    ...
    assert sum(1 for candidate in result.candidates[:5] if candidate.shape == "组三") <= 1
    assert all(6 <= candidate.sum_value <= 21 for candidate in result.candidates)
    assert all((candidate.span >= 1 if candidate.shape == "组三" else candidate.span >= 2) and candidate.span <= 9 for candidate in result.candidates)
...
def test_custom_fc3d_profile_keeps_explicit_model_weights():
    rule = get_lottery_rule("fc3d")
    config = DigitCandidateConfig(pair_weight=0.8, shape_weight=0.1, sum_weight=0.05, span_weight=0.02, omission_weight=0.2)
...
def test_default_pl5_candidates_exclude_latest_and_heavy_repeats():
    ...
    assert all(candidate.shape in {"全不同", "二一一一", "二二一", "三一一", "三二"} for candidate in result.candidates)
    assert sum(candidate.shape in {"三一一", "三二"} for candidate in result.candidates) <= 1
...
def test_digit_candidate_result_to_dict_is_report_friendly():
    ...
    assert payload["candidates"][0]["jointProbability"] == payload["candidates"][0]["compositeModelWeight"]
...
def test_three_digit_group3_allows_span_one_by_default():
    ...
    stats = analyze_digit_history(df, rule)
    config = DigitCandidateConfig(count=6, span_max=1, allowed_shapes=("组三",), exclude_latest=False)

    result = generate_digit_candidates(stats, rule, config=config, seed=1)

    assert len(result.candidates) == 6
    assert all(candidate.shape == "组三" and candidate.span == 1 for candidate in result.candidates)
...
def test_default_pl5_keeps_defensive_triple_shapes_as_small_quota():
    ...
    df = pd.DataFrame(
        [
            {"期数": f"2026{index:03d}", "万位": index % 10, "千位": (index + 1) % 10, "百位": (index + 2) % 10, "十位": (index + 3) % 10, "个位": (index + 4) % 10}
            for index in range(1, 31)
        ]
    )
    stats = analyze_digit_history(df, rule)

    result = generate_digit_candidates(stats, rule, config=DigitCandidateConfig(count=20), seed=3)

    defensive = [candidate for candidate in result.candidates if candidate.shape in {"三一一", "三二"}]
    assert 1 <= len(defensive) <= 4
    assert sum(candidate.shape in {"三一一", "三二"} for candidate in result.candidates[:5]) <= 1
...
def test_default_pl5_candidates_cover_at_least_half_digits_per_position():
    ...
    result = generate_digit_candidates(stats, rule, config=DigitCandidateConfig(count=10))

    for position in range(5):
        assert len({candidate.numbers[position] for candidate in result.candidates}) >= 6
...
def test_full_space_scoring_is_deterministic_and_not_limited_by_top_digits():
    ...
    stats = analyze_digit_history(df, rule)
    config = DigitCandidateConfig(count=8, top_digits_per_position=1, exclude_latest=False)
    ...
    assert [candidate.text for candidate in first.candidates] == [candidate.text for candidate in second.candidates]
    unique_by_position = [
...
def test_diversity_never_selects_below_configured_score_floor():
    ...
    result = generate_digit_candidates(stats, rule, config=config)
    score_only = generate_digit_candidates(stats, rule, config=replace(config, diversity_weight=0.0))
    scores = [candidate.score for candidate in result.candidates]

    assert min(scores) >= max(scores) - config.score_floor - 1e-9
    assert min(scores) >= min(candidate.score for candidate in score_only.candidates) - 1e-9
...
def test_default_score_floor_builds_a_strict_quality_pool():
    ...
    df = pd.DataFrame(
        [
            {"期数": str(2026001 + index), "百位": index % 3, "十位": (index + 1) % 4, "个位": (index + 2) % 5}
            for index in range(80)
        ]
    )
    stats = analyze_digit_history(df, rule)
    ...
    assert any(candidate.score < ranked[0].score - config.score_floor for candidate in ranked)
...
def test_three_digit_statistical_and_random_candidates_share_exact_shape_budget(
    ...
    df = pd.DataFrame(
        [
            {"期数": str(2026001 + index), "百位": index % 10, "十位": (index * 3 + 1) % 10, "个位": (index * 7 + 2) % 10}
            for index in range(80)
        ]
    )
    stats = analyze_digit_history(df, rule)
    ...
    statistical = generate_digit_betting_candidates(stats, rule, config=config, group_count=count)
    random_plan = generate_uniform_digit_betting_candidates(stats, rule, config=config, group_count=count, seed=7)

    for plan in (statistical, random_plan):
        assert len(plan.direct_candidates) == count
        assert len(plan.group_candidates) == count
        assert sum(candidate.shape == "组六" for candidate in plan.direct_candidates) == expected_group6
        assert sum(candidate.shape == "组三" for candidate in plan.direct_candidates) == expected_group3
        assert sum(candidate.shape == "组六" for candidate in plan.group_candidates) == expected_group6
        assert sum(candidate.shape == "组三" for candidate in plan.group_candidates) == expected_group3
...
def test_pl5_statistical_and_random_candidates_share_exact_defensive_budget(count):
    ...
    random_result = generate_uniform_digit_candidates(stats, rule, config=config, seed=11)
    expected_defensive = min(3, int(count * 0.15))

    for result in (statistical, random_result):
        defensive = sum(candidate.shape in {"三一一", "三二"} for candidate in result.candidates)
...
def test_three_digit_betting_candidates_aggregate_unique_group_probability_mass():
    ...
    df = pd.DataFrame(
        [
            {"期数": str(2026001 + index), "百位": index % 4, "十位": (index + 1) % 4, "个位": (index + 2) % 4}
            for index in range(60)
        ]
    )
    stats = analyze_digit_history(df, rule)

    plan = generate_digit_betting_candidates(stats, rule, config=DigitCandidateConfig(count=20), group_count=10)

    assert plan.candidates == plan.direct_candidates
    assert len({candidate.group_key for candidate in plan.group_candidates}) == len(plan.group_candidates)
    assert all(candidate.shape in {"组六", "组三"} for candidate in plan.group_candidates)
...
def test_pl3_and_pl5_share_identical_three_digit_prefix_score():
    ...
    pl5_rule = get_lottery_rule("pl5")
    pl3_df = pd.DataFrame(
        [{"期数": row["期数"], "百位": row["a"], "十位": row["b"], "个位": row["c"]} for row in rows]
    )
    pl5_df = pd.DataFrame(
        [
            {
    ...
    config = DigitCandidateConfig(frequency_windows=(20, 50), frequency_window_weights=(0.6, 0.4))
    pl3_stats = analyze_digit_history(pl3_df, pl3_rule, frequency_windows=config.frequency_windows)
    pl5_stats = analyze_digit_history(pl5_df, pl5_rule, frequency_windows=config.frequency_windows)

    candidates = ([1, 4, 9], [3, 0, 7], [8, 5, 2])
    pl3_scores = [score_digit_prefix(pl3_stats, numbers, config) for numbers in candidates]
    pl5_scores = [score_digit_prefix(pl5_stats, numbers, config) for numbers in candidates]
...
def test_candidate_config_validates_joint_weights_and_window_lengths():
    with pytest.raises(ValueError, match="窗口"):
        DigitCandidateConfig(frequency_windows=(20,), frequency_window_weights=(0.5, 0.5))
...
def test_uniform_three_digit_betting_plan_has_unique_group_keys_and_same_quota():
    ...
    df = pd.DataFrame(
        [
            {"期数": str(2026001 + index), "百位": index % 10, "十位": (index + 3) % 10, "个位": (index + 6) % 10}
            for index in range(30)
        ]
    )
    stats = analyze_digit_history(df, rule)
