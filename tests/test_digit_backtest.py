...
def test_fc3d_backtest_counts_direct_and_group_hits():
    ...
    result = DigitCandidateResult(
        "fc3d",
        "福彩3D",
        [_candidate("123"), _candidate("321"), _candidate("112")],
        DigitCandidateConfig(count=3),
    )
...
def test_pl5_backtest_only_counts_direct_hits():
    ...
    result = DigitCandidateResult(
        "pl5",
        "排列五",
        [_candidate("01234"), _candidate("43210"), _candidate("99887")],
        DigitCandidateConfig(count=3),
    )
...
def test_build_digit_backtest_markdown_contains_summary():
    ...
    result = DigitCandidateResult(
        "fc3d", "福彩3D", [_candidate("123")], DigitCandidateConfig(count=1)
    )

    markdown = build_digit_backtest_markdown(
        backtest_digit_candidates(df, rule, result)
    )
