...
from src.analysis.digit_report import build_digit_report_data, build_digit_report_markdown, generate_digit_report_from_csv
...
def test_build_digit_report_data_is_json_friendly():
    ...
    stats = analyze_digit_history(df, rule)
    candidates = generate_digit_candidates(stats, rule, config=DigitCandidateConfig(count=3), seed=1)
    backtest = backtest_digit_candidates(df, rule, candidates)

    payload = build_digit_report_data(stats, candidates, backtest, markdown_path=Path("reports/fc3d_daily_2026002.md"))
...
def test_generate_digit_report_from_csv_writes_json_when_enabled(tmp_path: Path):
    ...
    output = generate_digit_report_from_csv("fc3d", csv_path, output_dir=output_dir, write_json=True, candidate_count=3)
