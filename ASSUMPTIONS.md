...
- `behavioral_context_v1`累计频率压力协议已经停用；`behavioral_context_v2`固定A核心、B标准化自由行为、C标准化单调风险三组，C是唯一主挑战组且全部风险权重不得为正。行为特征初始权重0、2倍L2、10期半衰；不得在B/C中事后挑赢家。
- 行为形态特征只能表达相对`72%/27%/1%`理论占比的近期拥挤和连开超额风险；Top50平均形态总变差超过10%时禁止晋级。试机号必须具备`issue/trial_number/published_at/source/source_sha256`且严格早于正式开奖，否则保持不可用。
- 行为特征相对当期Top50边界的合计贡献必须为正且在全部完整500期块中为正；同一开发区逐项归因只能用于拒绝或提出未来挑战，不能据此删特征、反转权重后重新包装为独立验证。
...
- 锁定影子状态只允许由源码指纹和内容指纹完全匹配的代码继续增量；源码变化后必须写入新状态路径，旧状态仅保留审计。
