...
`behavioral_context_v1`是独立近期行为挑战器，默认不改变现有11维模型或锁定影子状态。B组在A组上增加完整号码/组选近期压力、上期同位/数字重合、形态转移偏离和形态连开长度；新权重从0开始并使用10倍L2。500期最小验收中，fc3d由`24/500=4.8%`降为`23/500=4.6%`，pl3由`35/500=7.0%`降为`31/500=6.2%`；两彩种配对LogLoss/Brier改善均`p>0.33`，Top50、25%提升、三时间块和形态联合闸门失败，因此不创建新影子状态、不替换当前日常模型。试机号C/D组因缺少带可靠开奖前时间戳的历史数据保持`trial_data_unavailable`。
